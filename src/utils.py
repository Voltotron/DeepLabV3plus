...
class SaveBestModel:
    """
    This class saves the best model based on the validation IoU.
    """
    def __init__(self, best_valid_iou=0.0, out_dir='../outputs', name='model'):
        self.out_dir = out_dir
        self.model_path = os.path.join(out_dir, f'best_{name}.pth')
        
        # Load previous best IoU if model checkpoint exists.
        if os.path.exists(self.model_path):
            checkpoint = torch.load(self.model_path)
            if 'best_valid_iou' in checkpoint:
                self.best_valid_iou = checkpoint['best_valid_iou']
            else:
                self.best_valid_iou = best_valid_iou
        else:
            self.best_valid_iou = best_valid_iou

    def __call__(self, current_valid_iou, epoch, model):
        if current_valid_iou > self.best_valid_iou:
            self.best_valid_iou = current_valid_iou
            print(f"\nBest validation IoU: {self.best_valid_iou:.4f}")
            print(f"\nSaving best model for epoch: {epoch+1}\n")
            torch.save({
                'epoch': epoch+1,
                'model_state_dict': model.state_dict(),
                'best_valid_iou': self.best_valid_iou,  # Save best IoU
            }, self.model_path)
...
def save_plots(train_acc, valid_acc, train_loss, valid_loss, train_iou, valid_iou, out_dir):
    """
    ...
    plt.figure(figsize=(10, 7))
    plt.plot(
        train_iou, color='tab:blue', linestyle='-', 
        label='train IoU'
    )
    plt.plot(
        valid_iou, color='tab:red', linestyle='-', 
