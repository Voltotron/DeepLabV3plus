...
from metrics import pix_acc, iou
...
def train(
    ...
    for i, data in enumerate(prog_bar):
        ...
        # For IoU (only for the concrete class, index 1).
        batch_iou = iou(outputs, target, class_index=1)
        train_running_iou.append(batch_iou)
        #############################
        ...
        prog_bar.set_description(desc=f"Loss: {loss.detach().cpu().numpy():.4f} | PixAcc: {train_running_pixacc.cpu().numpy()*100:.2f} | IoU: {batch_iou:.4f}")
    ...
    # Mean IoU for the concrete class
    mean_train_iou = np.nanmean(train_running_iou)
    ##############################
    return train_loss, pixel_acc, mean_train_iou
...
def validate(
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(prog_bar):
            ...
            # For IoU (only for the concrete class, index 1).
            batch_iou = iou(outputs, target, class_index=1)
            valid_running_iou.append(batch_iou)
            #############################

            prog_bar.set_description(desc=f"Loss: {loss.detach().cpu().numpy():.4f} | PixAcc: {valid_running_pixacc.cpu().numpy()*100:.2f} | IoU: {batch_iou:.4f}")
    ...
    # Mean IoU for the concrete class
    mean_valid_iou = np.nanmean(valid_running_iou)
    ##############################
    return valid_loss, pixel_acc, mean_valid_iou